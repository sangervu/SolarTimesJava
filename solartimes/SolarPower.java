...
public class SolarPower {
    ...
    public SolarPower(double latitudeDeg, double T, double currentSunElevationDeg, double maxSunElevationDeg) {
        ...
        this.maxSunPowerAnnual = MathNew.roundDesimal_1(1350.0 * Math.sin(maxSunAltitudeAnnualRad) * MathNew.pow(0.78, (1 / Math.sin(maxSunAltitudeAnnualRad))));

        // Maximun diurnal solar power
        double sunPower = MathNew.roundDesimal_1(1350.0 * Math.sin(maxSunAltitudeDiurnalRad) * MathNew.pow(0.78, (1 / Math.sin(maxSunAltitudeDiurnalRad))));
        if (sunPower < 0) {
        ...
        sunPower = MathNew.roundDesimal_1(1350.0 * Math.sin(currentSunElevationRad) * MathNew.pow(0.78, (1 / Math.sin(currentSunElevationRad))));
        if (sunPower < 0) {
